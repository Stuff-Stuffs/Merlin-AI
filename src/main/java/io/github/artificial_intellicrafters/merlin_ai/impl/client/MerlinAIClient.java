...
public class MerlinAIClient implements ClientModInitializer {
	...
	public void onInitializeClient(final ModContainer mod) {
		ClientChunkEvents.CHUNK_LOAD.register((world, chunk) -> ((ChunkPathingInfoImpl) ((AIWorld) world).merlin_ai$getChunkGraph()).load(chunk));
		ClientChunkEvents.CHUNK_UNLOAD.register((world, chunk) -> ((ChunkPathingInfoImpl) ((AIWorld) world).merlin_ai$getChunkGraph()).unload(chunk));
